...
class CUBDataset(Dataset):
    ...
    def __len__(self):
        ...
        self.dataset_keys = [str(k) for k in file[self.split].keys()]
    ...
    def __getitem__(self, idx):
        ...
        correct_image = self.qualityInsurance(correct_image)
        incorrect_image = self.qualityInsurance(incorrect_image)

        sample = {'correct_image': torch.FloatTensor(correct_image), 'correct_embed': torch.FloatTensor(correct_embed),
                   'incorrect_image': torch.FloatTensor(incorrect_image), 'inter_embed': torch.FloatTensor(inter_embed), 'caption': str(caption)}
        
        sample['correct_image'] = sample['correct_image'].sub_(127.5).div_(127.5)
        sample['incorrect_image'] =sample['incorrect_image'].sub_(127.5).div_(127.5)
